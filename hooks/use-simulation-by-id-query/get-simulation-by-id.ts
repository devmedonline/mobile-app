...
export type GetSimulationByIdParams = { id: string };
...
const FAKE_SIMULATION: Simulation = {
  id: 1,
  ...
  initialPhase: 3,
  phases: yolo.range(1, 5).map((j) => ({
    id: j,
    title: yolo.faker.lorem.words(3),
    description: yolo.faker.lorem.paragraph(),
    order: j,
    content,
  })),
};
...
export async function getSimulationById({
  id,
}: GetSimulationByIdParams): Promise<Simulation> {
